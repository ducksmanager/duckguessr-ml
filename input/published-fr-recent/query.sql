...
FROM duckguessr_published_fr_recent_game_all entryurl_ids
     ...
     INNER JOIN
     (SELECT artist_with_2010s_entries.personcode, nationalitycountrycode, fullname
      FROM duckguessr_published_fr_recent_game_all artist_with_2010s_entries
           INNER JOIN inducks_person person ON artist_with_2010s_entries.personcode = person.personcode
      GROUP BY artist_with_2010s_entries.personcode
      HAVING COUNT(*) >= 20
